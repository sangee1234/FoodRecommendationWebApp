...
def test_format_records():
    try:
        ...
        pyc_records = [_change_file_extensions_to_pyc(record) for record in records]
        ...
        arrow_regex = r"^-+>\s+\d+\s+"
        assert re.search(
            arrow_regex + r"_raise_exception\('a', 42\)",
            formatted_records[0],
            re.MULTILINE,
        )
        assert re.search(
            arrow_regex + r"helper\(a, b\)", formatted_records[1], re.MULTILINE
        )
        assert "a = 'a'" in formatted_records[1]
        assert "b = 42" in formatted_records[1]
        assert re.search(
            arrow_regex + r"raise ValueError\('Nope, this can not work'\)",
            formatted_records[2],
            re.MULTILINE,
        )
...
def test_format_records_file_with_less_lines_than_context(tmpdir):
    ...
    try:
        ...
        # Check exception stack
        arrow_regex = r"^-+>\s+\d+\s+"
        assert re.search(arrow_regex + r"1/0", formatted_records[1], re.MULTILINE)
...
def test_format_exc_with_compiled_code():
    ...
    try:
        ...
        formatted_exc = format_exc(exc_type, exc_value, exc_traceback, context=10)
