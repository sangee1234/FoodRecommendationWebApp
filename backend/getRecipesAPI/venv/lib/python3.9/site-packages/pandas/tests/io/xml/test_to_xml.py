...
@pytest.mark.skipif(
    not PY38, reason=("etree alpha ordered attributes < py 3.8"),
)
...
@pytest.mark.skipif(
    not PY38, reason=("etree alpha ordered attributes < py3.8"),
)
...
def test_stylesheet_wrong_path(datapath):
    ...
    with pytest.raises(
        XMLSyntaxError, match=("Start tag expected, '<' not found"),
    ):
...
def test_compression_output(parser, comp):
    with tm.ensure_clean() as path:
        ...
        with get_handle(path, "r", compression=comp,) as handle_obj:
