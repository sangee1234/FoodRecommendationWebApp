...
class Block(PandasObject):
    ...
    def replace(
        ...
        if not mask.any():
            ...
            blk = self.coerce_to_target_dtype(value)
            return blk.replace(
                to_replace=to_replace, value=value, inplace=True, regex=regex,
            )
    ...
    @final
    def _replace_regex(
        self, to_replace, value, inplace: bool = False, convert: bool = True, mask=None,
    ) -> list[Block]:
        """
        ...
        for i, (src, dest) in enumerate(pairs):
            ...
            for blk_num, blk in enumerate(rb):
                ...
                result = blk._replace_coerce(
                    to_replace=src, value=dest, mask=m, inplace=inplace, regex=regex,
                )
            ...
            if not regex:
                ...
                if regex:
                    return self._replace_regex(
                        to_replace, value, inplace=inplace, convert=False, mask=mask,
                    )
