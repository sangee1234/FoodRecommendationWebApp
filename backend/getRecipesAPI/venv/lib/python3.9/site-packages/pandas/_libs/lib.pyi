...
def astype_intsafe(
    arr: np.ndarray, new_dtype: np.dtype,  # np.ndarray[object]
) -> np.ndarray: ...
...
def maybe_indices_to_slice(
    indices: np.ndarray, max_len: int,  # np.ndarray[np.intp]
) -> slice | np.ndarray: ...  # np.ndarray[np.uint8]
...
def get_level_sorter(
    label: np.ndarray, starts: np.ndarray,  # const int64_t[:]  # const intp_t[:]
...
def array_equivalent_object(
    left: np.ndarray, right: np.ndarray,  # object[:]  # object[:]
) -> bool: ...
