...
def svds(
    ...
    if solver == "lobpcg":
        ...
        if not HAS_PROPACK:
            raise ValueError(
                "`solver='propack'` is opt-in due to potential issues on Windows, "
    ...
    u = (
        _augmented_orthonormal_cols(ularge, nsmall, random_state)
        if ularge is not None
        else None
    )
    vh = (
        _augmented_orthonormal_rows(vhlarge, nsmall, random_state)
