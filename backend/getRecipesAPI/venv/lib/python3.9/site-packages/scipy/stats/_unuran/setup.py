...
def unuran_pre_build_hook(build_clib, build_info):
    from scipy._build_utils.compiler_helper import get_c_std_flag, try_compile, has_flag
    ...
    deps = {
        ...
        "sys/time.h": ["HAVE_GETTIMEOFDAY", "HAVE_SYS_TIME_H", "TIME_WITH_SYS_TIME"],
...
def _get_sources(dirs):
    sources = []
    for dir_ in dirs:
        files = [file for file in os.listdir(dir_) if (not os.path.isdir(file))]
...
def configuration(parent_package="", top_path=None):
    ...
    if not os.path.exists(_unuran_dir(ret_path=True) / "README.md"):
        raise RuntimeError(
            "Missing the `unuran` submodule! Run `git "
